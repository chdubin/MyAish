...
<%
    DateTime start_date = new DateTime(2012, 10, 12);
    DateTime end_date = new DateTime(2012, 10, 13);

    TimeSpan start_time = new TimeSpan(17, 45, 0);
    TimeSpan end_time = new TimeSpan(18, 45, 0);
%>
                <% if ((DateTime.Now.TimeOfDay.CompareTo(start_time) >= 0 && DateTime.Now.Date.CompareTo(start_date) == 0)
                       ||
                       (DateTime.Now.TimeOfDay.CompareTo(end_time) <= 0 && DateTime.Now.Date.CompareTo(end_date) == 0))
                   { %>
...
border-image: initial;margin-bottom:10px;">
        ...
        <% int st_hrs = (start_time.Hours > 12 ? start_time.Hours - 12 : start_time.Hours) + 1;
        int end_hrs = (end_time.Hours > 12 ? end_time.Hours - 12 : end_time.Hours) + 1; %>
        Due to observance of Shabbat, the shopping cart will not be active <br /><br />from <%= start_date.ToString("d") %> <%= st_hrs %>:<%= start_time.Minutes < 10 ? String.Format("0{0}",start_time.Minutes) : start_time.Minutes.ToString() %> PM EST until <%= end_date.ToString("d") %> <%= end_hrs %>:<%= end_time.Minutes < 10 ? String.Format("0{0}",end_time.Minutes) : end_time.Minutes.ToString() %> PM EST<br /><br /> 
        Thank you!
     </p>
</div>
                            ...
                            <td colspan="2">
                                <label>
                                    <input id="billing_address_cb" type="radio" onclick="$('.shipping').val('');$('#shipping_information').hide();$('#UseStandaloneShippingAddress').attr('value', false)"
                                        checked="checked" value="billing" name="shipto" />
                                    Ship to my billing address, or mp3 download (no shipping).</label>
                                <br />
                                <label>
