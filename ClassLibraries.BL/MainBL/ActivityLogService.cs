...
namespace MainBL
{
    public class ActivityLogService : BaseBO, IActivityLogService
    {
       ...
       // this was added on Feb 2 from version of project i had downloaded from github
        public RoyaltyLog[] SelectUserIDsByClassLog(long classId, ActivityLogTypeEnum[] activity_types, DateTime startDate, DateTime endDate)
        {
            return this.Exec(System.Data.IsolationLevel.Snapshot,
              (ActivityLogDbManager context) =>
              {
                  return context.SelectUserIDsByClassLog(classId, activity_types, startDate, endDate).ToArray();
              });
        }
