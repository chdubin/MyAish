...
namespace MainBL
{
    public partial class ClassService : BaseBO, IClassService
    {
        ...
        private static void LoadClassData(ClassDbManager context, ClassEntity[] classes)
        {
          ...
          //this was addred on feb 2 from version i  had download from github
	  var fileData = context.GetFilesForClasses(classIDs);
      if (classes.Length > 0)
      {
          foreach (var c in classes)
          {
              ...
              {
                  c.FilePath = fileData[0].FileEntity.filePath;
              }
          }
      }
        }
