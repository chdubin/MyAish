...
namespace MainEntity
{
    ...
    // this was added on feb 2 from version i had download from git hube
        public IQueryable<MainEntity.Models.Activity.RoyaltyLog> SelectUserIDsByClassPurchase(long classId, short[] shopping_states, DateTime startDate, DateTime endDate)
        {
            var shop = from sh in this.Shopping
                       join tl in this.ShoppingTransaction on sh.shoppingTransactionID equals tl.shoppingTransactionID
                       join mem in this.Membership on sh.UserId equals mem.UserId
                       join ei in this.EntityItems on sh.entityID equals ei.entityID
                       where ei.parentEntityID == classId
                       && shopping_states.Contains(sh.shoppingStateID)
                       && !sh.deleted
                       && (tl.createDate >= startDate && tl.createDate < endDate.AddDays(1))
                       select new MainEntity.Models.Activity.RoyaltyLog { ClassID = sh.entityID, UserId = mem.UserId, Title = ei.title, FirstName = mem.firstName, LastName=mem.lastName, Price = (sh.price1 == 0 ? sh.price2 : sh.price1) };
