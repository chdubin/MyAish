...
namespace MainEntity
{
    public partial class FileDbManager
    {
      ...
      // this was added on feb 2 from version i had download from git hub
        public IQueryable<FileRoyaltiesEntity> SelectFilesByRoyalties(MainEntity.Models.Activity.RoyaltyLog[] files, List<MainEntity.Models.File.FileRoyaltiesEntity> fileList)
        {
            List<long> newFileIds = new List<long>();
            foreach (Models.Activity.RoyaltyLog log_ in files.Distinct())
            {
                bool addId = fileList == null;
                if (!addId)
                {
                    FileRoyaltiesEntity file = fileList.FirstOrDefault(x => x.fileId == log_.ClassID && x.UserId == log_.UserId);
                    addId = file == null;
                }
                if (addId)
                    newFileIds.Add(log_.ClassID);
            }
            var rval = from f in this.FileEntity
                       join e in this.EntityItem on f.fileID equals e.entityID
                       join c in this.ClassEntity on e.parentEntityID equals c.classID
                       ...
                       select new FileRoyaltiesEntity()
                       {
                           fileId = f.fileID
                       };
